...
import { getPreset, listPresets, type PresetProfile } from '../presets/profiles.js';
...
export async function presetCommand(name: string | undefined, options: PresetOptions): Promise<void> {
    ...
    if (preset.skills.length > 0) logger.dim(`  • ${preset.skills.length} skill(s)`);
    if (preset.hooks.length > 0) logger.dim(`  • ${preset.hooks.length} hook preset(s)`);
    if (preset.mcpServers.length > 0) logger.dim(`  • ${preset.mcpServers.length} MCP server(s)`);
    if (preset.commands.length > 0) logger.dim(`  • ${preset.commands.length} slash command(s)`);
    if (preset.rules.length > 0) logger.dim(`  • ${preset.rules.length} rule(s)`);
    console.log('');
...
async function applyPreset(preset: PresetProfile): Promise<void> {
