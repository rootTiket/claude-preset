...
import { getSkillTemplate, TEMPLATE_DESCRIPTIONS, type SkillTemplate } from '../templates/skill-templates.js';
...
export async function addSkillCommand(name: string, options: AddSkillOptions): Promise<void> {
    if (!isClaudeProject()) {
            ...
            : [
                {
                    ...
                    validate: (input: string) => {
                        ...
                        if (input.length > 1024) return 'Must be under 1024 characters';
                        if (/<[^>]+>/.test(input)) return 'Must not contain XML tags (< >)';
                        return true;
                    },
                },
            ]),
    ...
    const template = (options.template ?? answers.template) as SkillTemplate;
    ...
    if (answers.createScripts) ensureDir(path.join(skillDir, 'scripts'));
...
async function checkExists(skillDir: string): Promise<boolean> {
