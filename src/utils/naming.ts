...
export function validateSkillName(name: string): { valid: boolean; errors: string[] } {
    ...
    if (!isKebabCase(name)) {
        ...
        if (/[A-Z]/.test(name)) errors.push('  → No capital letters allowed');
        if (/\s/.test(name)) errors.push('  → No spaces allowed');
        if (/_/.test(name)) errors.push('  → No underscores allowed (use hyphens)');
    }
...
export function validateDescription(description: string): { valid: boolean; errors: string[]; warnings: string[] } {
    const errors: string[] = [];
