...
import { getClaudeDir, isClaudeProject, mergeJSON } from '../utils/file-ops.js';
import { MCP_PRESETS, getMcpPreset } from '../templates/mcp-presets.js';
import { logger } from '../utils/logger.js';
export async function addMcpCommand(name, options) {
    if (!isClaudeProject()) {
...
async function applyMcpPreset(presetName) {
    ...
    if (preset.envVarsNeeded.length > 0) {
        ...
        const envAnswers = await inquirer.prompt(preset.envVarsNeeded.map((envVar) => ({
            type: 'input',
            name: envVar,
            message: `${envVar}:`,
            default: '',
...
async function addCustomMcp(name) {
    const answers = await inquirer.prompt([
        { type: 'input', name: 'command', message: 'Command to run:', default: 'npx' },
        {
            ...
            filter: (input) => input.split(/\s+/).filter(Boolean),
        },
        {
            type: 'confirm',
            name: 'hasEnv',
