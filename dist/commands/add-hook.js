...
import { HOOK_EVENTS, HOOK_PRESETS, getHookPreset } from '../templates/hooks-presets.js';
import { logger } from '../utils/logger.js';
export async function addHookCommand(options) {
    if (!isClaudeProject()) {
    ...
    let handler = { type: answers.handlerType };
    ...
    if (addMatcher) {
        ...
        matcher = {};
        if (matcherAnswers.tool_name)
            matcher.tool_name = matcherAnswers.tool_name;
        if (matcherAnswers.file_pattern)
            matcher.file_pattern = matcherAnswers.file_pattern;
        if (matcherAnswers.command_pattern)
            matcher.command_pattern = matcherAnswers.command_pattern;
    ...
    if (matcher)
        hookDef.matcher = matcher;
    const settingsPath = path.join(getClaudeDir(), 'settings.json');
    const existing = readJSON(settingsPath) ?? {};
    const hooks = existing.hooks ?? {};
    if (!hooks[answers.event])
        hooks[answers.event] = [];
    hooks[answers.event].push(hookDef);
...
async function applyHookPreset(presetName) {
